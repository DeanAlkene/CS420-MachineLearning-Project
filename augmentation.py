...
def augmentation(path_aug='./results/aug'):
    # read images
    train_path = "./data/new_train_set/train_img"
    label_path = "./data/new_train_set/train_label"
    img_type = "png"
   ...
   # This will do preprocessing and realtime data augmentation:
    ...
    for i in range(slices):
        img_t = load_img(train_path+"/"+str(i)+".png", grayscale=True)
        img_l = load_img(label_path+"/"+str(i)+".png", grayscale=True)
        ...
        batches = 0
        for batch in datagen.flow(img, batch_size=1, save_to_dir=path_aug, save_prefix=str(i), save_format='png'):
            batches += 1
            if batches >= 30:
                break
            # we need to break the loop by hand because
            # the generator loops indefinitely
    aug_imgs = glob.glob(path_aug+"/*.png")
    ...
    for imgname in aug_imgs:
        ...
        img_train.save(path_aug+"/train/"+str(i)+".png")
        img_label.save(path_aug+"/label/"+str(i)+".png")
        i += 1
